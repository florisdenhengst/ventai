...
greedy_unsafe = utils.repair_unsupported_greedy_policy(
    joblib.load(greedy_policy_file.format(seed, shaping, shaping_scalar, unsafety_prob)),
    behavior_train_policy
)
...
if train_test == 'test':
    evaluations = [
                 (test_set, greedy_unsafe, behavior_policy, 'test', 'greedy', shaped, shaping, shaping_scalar, 'unsafe', unsafety_prob, seed),
...
if unsafety_prob == 1.0 and not SOLOTEST:
    if train_test == 'test':
        evaluations += [
                (test_set, greedy_safe, behavior_policy, 'test', 'greedy', shaped, shaping, shaping_scalar, 'safe', unsafety_prob, seed),
            ]
    else:
        evaluations += [
                (train_set, greedy_safe, behavior_policy, 'train', 'greedy', shaped, shaping,  shaping_scalar, 'safe', unsafety_prob, seed),
                ]
#
#if shaping == 'none':
